...
public class MadminFrm implements ActionListener {
	String cksql="select * from admin";
	JFrame w;
	JDialog jdl=new JDialog(w,"用户信息管理",true);
	//显示信息的表
	String[] stafflistcols={"id","账号","密码"};
	//设置表格整行显示
	...
	MadminFrm(JFrame jf){
		...
		stafflist.addMouseListener(new java.awt.event.MouseAdapter(){
            public void mouseClicked(MouseEvent e) {
              ...
              mgpasswordtxt.setText(stafflist.getValueAt(r, 2).toString());
            }
        });
		//为增删改重置功能添加监听事件
	...
	class AddAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if("".equals(sno)||"".equals(password)){
				JOptionPane.showMessageDialog(null,"用户信息不完整!", "系统信息", JOptionPane.WARNING_MESSAGE);	
				return;
			}
			DbHelper db=new DbHelper();
			if(db.checkTrue("select id from admin where uname='"+sno+"'")){
				JOptionPane.showMessageDialog(null,"账号已存在!", "系统信息", JOptionPane.WARNING_MESSAGE);	
				return;
			}
			int x=db.executeUpdate("insert into admin(uname,upassword) values('"+sno+"','"+password+"')");
			if(x>0){
	...
	class UpdAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			int x=db.executeUpdate("update admin set uname='"+sno+"',upassword='"+password+"' where id="+id);
			if(x>0){
	...
	public void getData(String sql){
		...
		if(rs!=null){
			try {
				while(rs.next()){
					model.addRow(new String[]{String.valueOf(rs.getInt("ID")),rs.getString("uname"),rs.getString("upassword")});
				}
			} catch (SQLException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
