...
public class MshuiweiFrm implements ActionListener {
	String cksql="select infor.* from infor where 1=1";
	JFrame w;
	JDialog jdl=new JDialog(w,"水位信息管理",true);
	//显示信息的表
	String[] stafflistcols={"id","塔号","日期","水位值"};
	//设置表格整行显示
	...
	JLabel mgsnamelb=new JLabel("日期：");
	JTextField mgsnametxt=new JTextField();
	JLabel mgclsslb=new JLabel("水位值：");
	...
	class AddAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(db.checkTrue("select id from infor where sno='"+sno+"'")){
				JOptionPane.showMessageDialog(null,"塔号已存在!", "系统信息", JOptionPane.WARNING_MESSAGE);	
				return;
			}
			int x=db.executeUpdate("insert into infor(sno,date,shuiwei) values('"+sno+"','"+sname+"','"+clssname+"')");
			if(x>0){
	...
	class DelAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			DbHelper db=new DbHelper();
			if("".equals(id)||!db.checkTrue("select id from infor where id="+id)){
				JOptionPane.showMessageDialog(null,"请选择所要删除的信息!", "系统信息", JOptionPane.WARNING_MESSAGE);	
				return;
			}			
			int x=db.executeUpdate("delete from infor where id="+id);
			if(x>0){
	...
	class UpdAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			DbHelper db=new DbHelper();
			rs=db.executeQuery("select id from infor where shuiwei='"+sno+"'");
			try {
				while(rs.next()){
			...
			int x=db.executeUpdate("update infor set sno='"+sno+"',date='"+sname+"',shuiwei='"+clssname+"' where id="+id);
			if(x>0){
	...
	public void getData(String sql){
		...
		if(!"".equals(ckclsstxt.getText().toString())){
			sql+=" and date like '%"+ckclsstxt.getText()+"%'";
		}
		...
		if(rs!=null){
			try {
				while(rs.next()){
					model.addRow(new String[]{String.valueOf(rs.getInt("ID")),rs.getString("sno"),rs.getString("date"),rs.getString("shuiwei")});
				}
			} catch (SQLException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
